...
values ('Анохина', 'Ирина', 'Прокопьевна', '1989-02-22', 'Сергиев Посад, Зеленый пер., д. 20, кв. 58',
       ...
       ('Дмитриев', 'Дмитрий', 'Дмитриевич', '1955-08-08', 'Уфа, ул. Чайковского 45д.10, кв.55',
...
select                           concat(V.Make, ' ', V.Model)                                 as MakeModel,
    ...
    'drunk'                    = (case the_driver_is_drunk when 1 then 'Да' when 0 then 'Нет' end),
    ...
    'is_the_vehicle_insured'   = (case I.casco when 1 then 'Да' when 0 then 'Нет' end),
    'damage_to_other_property' = (case RD.damage_to_other_property when 1 then 'Да' when 0 then 'Нет' end),
    'Damaged_property_name'    = (case RD.damage_to_other_property when 1 then RD.Damaged_property_name else '–' end),
    'Damaged_property_owner'   = (case RD.damage_to_other_property when 1 then RD.Damaged_property_owner else '–' end),
    'can_the_vehicle_move'     = (case RD.can_the_vehicle_move when 1 then 'Да' when 0 then 'Нет' end),
    'Vehicle_parking_address'  = (case RD.can_the_vehicle_move when 1 then '–' else RD.Vehicle_parking_address end)
...
as
select top 1 concat(Surname, ' ', Name, ' ', Patronymic) as FIO, convert(varchar(10), Date_of_birth, 104) as Date_of_birth, Address, Phone_number
from Witness
where RTA_id = @rtaid
order by 1 asc
go